/// <reference types="npm:@types/react@18.3.1" />
import * as React from 'npm:react@18.3.1'
import { Text } from 'npm:@react-email/components@0.0.22'
import { SoloBizzShell, BrandButton, styles } from './brand.tsx'
import { getStrings, normalizeLang } from './i18n.ts'

interface EmailChangeEmailProps {
  siteName?: string
  siteUrl?: string
  recipient?: string
  email?: string
  oldEmail?: string
  newEmail?: string
  confirmationUrl: string
  language?: string
}

export const EmailChangeEmail = ({
  email,
  oldEmail,
  newEmail,
  confirmationUrl,
  language,
}: EmailChangeEmailProps) => {
  const T = getStrings(language).emailChange
  const F = getStrings(language).footer
  const from = oldEmail || email || ''
  const to = newEmail || ''
  return (
    <SoloBizzShell
      lang={normalizeLang(language)}
      preview={T.preview}
      icon={<>📧</>}
      title={T.heroTitle}
      subtitle={T.heroSub}
      tagline={T.tagline}
      footer={<Text style={styles.footerText}>{F.brandLine}</Text>}
    >
      <Text style={styles.paragraph}>{T.body(from, to)}</Text>
      <Text style={styles.paragraph}>{T.confirmIntro}</Text>
      <BrandButton href={confirmationUrl}>{T.cta} →</BrandButton>
      <Text style={{ ...styles.warnBox }}>{T.warning}</Text>
    </SoloBizzShell>
  )
}

export default EmailChangeEmail

...
DECLARE
  ...
  v_base_ts timestamp := CURRENT_DATE::timestamp + interval '7 hours';
...
BEGIN
  ...
  VALUES
    (p_user_id, 'Individual Session', 50, 50, true, v_seed, v_batch)
  RETURNING id INTO v_svc_individual;

  INSERT INTO public.services (user_id, name, price, duration_minutes, is_demo, seed_source, seed_batch_id)
  VALUES
    (p_user_id, 'Group Session', 100, 90, true, v_seed, v_batch)
  RETURNING id INTO v_svc_group;

  INSERT INTO public.services (user_id, name, price, duration_minutes, is_demo, seed_source, seed_batch_id)
  VALUES
    (p_user_id, 'Training', 20, 45, true, v_seed, v_batch)
  ...
  VALUES
    (p_user_id, 'Hogwarts Group Session', 'Demo group sessions inspired by the Harry Potter films.', 'active', true, true, false, true, v_seed, v_batch)
  RETURNING id INTO v_group_main;

  INSERT INTO public.groups (user_id, name, description, status, bill_present, bill_absent, bill_skipped, is_demo, seed_source, seed_batch_id)
  VALUES
    (p_user_id, 'Defence Training Circle', 'Demo training group for short practice sessions.', 'active', true, false, false, true, v_seed, v_batch)
  ...
  LOOP
    ...
    FOR v_i IN 1..20 LOOP
      ...
      VALUES (p_user_id, v_client_id, v_apt, 50, v_today, 'card', 'session', v_client.name || ' — Individual Session', true, v_seed, v_batch);

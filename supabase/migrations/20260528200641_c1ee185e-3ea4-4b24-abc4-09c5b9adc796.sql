-- Align public booking availability with internal Calendar:
-- ANY appointment that holds calendar time must block the slot.
-- Only fully-cancelled sessions free the slot. no-show / pending-confirmation /
-- completed sessions all reserve time and must therefore block public booking.
    ...
    LOOP
      ...
      LOOP
        ...
        IF v_slot >= v_min_from
           ...
           AND NOT EXISTS (
             -- ANY appointment that holds calendar time blocks the slot.
             -- Only 'cancelled' frees the slot. no-show / pending-confirmation
             -- / completed / scheduled all reserve time in the Calendar.
...
-- Mirror the same blocking rule in the server-side submit validation.

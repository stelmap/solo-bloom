...
BEGIN
  ...
  -- 1) Consume from explicit client_credits rows (legacy / explicit credit).
  FOR r IN
  ...
  -- 2) Fall back to any income with unallocated remainder. This handles the
  --    common case where a therapist recorded prepayment as an income without
  --    explicit per-session allocations: that surplus is effectively client
  --    credit and should be consumable on session completion.
  IF v_remaining > 0 THEN
    FOR r IN
      SELECT i.id AS income_id,
             Number(i.amount) - COALESCE((

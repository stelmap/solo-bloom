...
BEGIN
  ...
  SELECT public.client_prepaid_balance(p_client_id, v_user, p_appointment_id)
    INTO v_balance_before;

  SELECT cc.income_id
    INTO v_first_prepay_income
    FROM public.client_credits cc
    WHERE cc.client_id = p_client_id AND cc.user_id = v_user AND cc.income_id IS NOT NULL AND cc.amount > 0
  ...
  v_balance_after := GREATEST(0, v_balance_before - v_consumed);

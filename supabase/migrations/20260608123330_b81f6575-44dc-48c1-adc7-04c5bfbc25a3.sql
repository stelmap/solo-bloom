...
BEGIN
  ...
  -- Pull session duration + buffer from Public Booking settings first.
  -- Fall back to profile default, then to 60min/0min.
  ...
  WHILE v_day <= v_max_to LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        -- Advance by session duration + buffer so the next bookable slot
        -- only opens after the buffer has passed.
...
BEGIN
  ...
  -- Pull session duration + buffer from Public Booking settings.
  SELECT COALESCE(ba.session_duration_minutes, p.default_duration, 60),
         COALESCE(ba.buffer_minutes, 0)
    INTO v_dur, v_buffer
    FROM public.profiles p
  ...
  FOR v_rule IN
    SELECT start_time::time AS s, end_time::time AS e
     ...
     WHERE user_id = v_link.user_id
       ...
       AND is_working = true
  LOOP
    -- Slot must fit inside the working window (session, not buffer) AND
    -- must be aligned to the stride (duration + buffer) from the window start.

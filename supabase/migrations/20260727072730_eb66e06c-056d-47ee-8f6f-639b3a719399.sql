CREATE OR REPLACE FUNCTION public.admin_delete_user_and_data(p_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- ---- Guards: never proceed without a resolvable, safe target -------------
  ...
  -- ---- Child rows reachable only through this tenant's parents -------------
  ...
  -- ---- Tenant-owned rows, dependency order --------------------------------
  ...
  -- public.user_lifecycle_audit is intentionally preserved: it is the minimal
  -- admin-level record that a deletion happened (no client/business content).

  RETURN jsonb_build_object('ok', true, 'user_id', p_user_id, 'deleted_counts', v_counts);
END;
$$;

REVOKE ALL ON FUNCTION public.admin_delete_user_and_data(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_delete_user_and_data(uuid) TO service_role;